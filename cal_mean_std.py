...
def cal_mean_std(surface_folder_path = surface_folder_path, upper_folder_path = upper_folder_path):
    ...
    for i, file in enumerate(file_list):
        print("surface_mean", i)
        file_path = os.path.join(surface_folder_path, file)
        data_tensor = nc_surface_loader(file_path)      #  (day, 4, 1440, 721)      day 是一个月的天数
    ...
    # 标准差的计算
    surface_var_list = torch.zeros(len(file_list), 4)
    for i, file in enumerate(file_list):
        print("surface_std", i)
        file_path = os.path.join(surface_folder_path, file)
        data_tensor = nc_surface_loader(file_path)      #  (4, day, 1440, 721)
        
        surface_var_list[i] = torch.mean(((data_tensor.permute(1, 2, 3, 0) - all_surface_mean) ** 2), dim = (0, 1, 2))

    all_surface_std = torch.sqrt(torch.mean(surface_var_list, dim = 0))  
...
if __name__ == "__main__":
    ...
    torch.save(all_surface_mean, '/home/lbk/llm/llama4pde/data/surface_mean.pt')
    torch.save(all_surface_std, '/home/lbk/llm/llama4pde/data/surface_std.pt')
    torch.save(all_upper_mean, '/home/lbk/llm/llama4pde/data/upper_mean.pt')
    torch.save(all_upper_std, '/home/lbk/llm/llama4pde/data/upper_std.pt')
